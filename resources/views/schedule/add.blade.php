...
<form class="row g-3" method="post" action="{{ route('schedules.add') }}">
  @csrf
  <div class="col-md-3">
    <label for="inputDate" class="form-label">@lang('default.date')*</label>
    <input name="date" class="form-control" id="inputDate" required>
  </div>
  <div class="col-md-3">
    ...
    <select name="situation" id="inputSituation" class="form-select">
        @foreach($inputSituations as $key => $option)
            <option value="{{ $key }}" @if($key == 1) selected @endif>{{ $option }}</option>
        @endforeach
    </select>
  </div>
  <div class="col-12">
